...
export default function LelekaChat() {
  ...
  const [mobileView, setMobileView] = useState<'chats' | 'chat' | 'profile'>(
    'chats'
  );
  ...
  const [messages, setMessages] = useState<Record<string, Message[]>>({
    ...
    '5': [
      {
        id: 'm11',
        text: 'Переглянь мої нові фото 📸',
        fromMe: false,
        time: '16:45',
      },
      { id: 'm12', text: 'О, гарні! Де це?', fromMe: true, time: '16:55' },
    ],
    '6': [{ id: 'm13', text: 'Як справи?', fromMe: false, time: '08:20' }],
  });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-green-50">
      <header className="w-full h-16 bg-[#A1C78F] border-b border-black flex items-center justify-between px-4 relative">
        <div
          className={`flex items-center gap-3 ${mobileView !== 'chats' ? 'hidden md:flex' : ''}`}
        >
          <img
            src="/icons/menu.png"
        ...
        >
          {filtered.map((it, idx) => (
            ...
            >
              <img
                src="/icons/stork.png"
                className="w-12 h-12 bg-white border border-black rounded-full p-1 transition-transform hover:scale-110"
              />
              ...
              <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-3 w-full">
                {messages[selected.id]?.map((m) => (
                  ...
                  >
                    ...
                    <p className="text-xs opacity-70 mt-1 text-right text-black">
                      {m.time}
                    </p>
                  </div>
                ))}
        ...
        >
          <div className="p-6 flex flex-col items-center text-center">
            ...
            <p className="mt-4 text-gray-800 text-sm leading-5">
              Привіт! Це мій профіль у Лелеці. Люблю слухати музику, гуляти і
              знайомитись з новими людьми 🤍
            </p>
            <button
      ...
      >
        <ul className="flex flex-col text-black select-none">
          <li
            ...
            className="menu-item hover:bg-[#A4C894] transition-colors"
          >
            <img src="/icons/profile.png" /> Профіль
          </li>
          <li
            onClick={() => goTo('/contacts')}
            className="menu-item hover:bg-[#A4C894] transition-colors"
          >
            <img src="/icons/phone.png" /> Контакти
          </li>
          <li
            onClick={() => goTo('/createGroup')}
            className="menu-item hover:bg-[#A4C894] transition-colors"
          >
            <img src="/icons/add-group.png" /> Створити групу
          </li>
          <li
            onClick={() => goTo('/privateFolder')}
            className="menu-item hover:bg-[#A4C894] transition-colors"
          >
            <img src="/icons/folder.png" /> Приватна папка
          </li>
          <li
            onClick={() => goTo('/archive')}
            className="menu-item hover:bg-[#A4C894] transition-colors"
          >
            <img src="/icons/archive.png" /> Архів
          </li>
          <li
            onClick={() => goTo('/settings')}
            className="menu-item hover:bg-[#A4C894] transition-colors"
          >
            <img src="/icons/settings.png" /> Налаштування
          </li>
          <li className="menu-item hover:bg-[#A4C894] transition-colors">
            <img src="/icons/theme.png" /> Світла/темна тема
          </li>
          <li
            onClick={() => setLogoutModal(true)}
            className="px-4 py-3 text-red-600 hover:bg-[#A4C894] border-b border-black transition-colors cursor-pointer"
          >
      ...
      {selected && (
        ...
        >
          <ul className="flex flex-col text-sm text-black">
            <li className="menu-item flex justify-between hover:bg-[#A4C894] transition-colors">
              <span>Додати до контактів</span>
              <img src="/icons/Add-one.png" className="w-4" />
            </li>
            <li className="menu-item flex justify-between hover:bg-[#A4C894] transition-colors">
            ...
            <li className="menu-item flex justify-between hover:bg-[#A4C894] transition-colors">
              <span>Поділитись контактом</span>
              <img src="/icons/Share-two.png" className="w-4" />
            </li>
            <li className="menu-item flex justify-between hover:bg-[#A4C894] transition-colors">
              <span>Засекретити</span>
              <img src="/icons/Preview-close-one.png" className="w-4" />
            </li>
            <li className="menu-item flex justify-between hover:bg-[#A4C894] transition-colors">
              <span>Заблокувати</span>
              <img src="/icons/Forbid.png" className="w-4" />
            </li>
            <li className="menu-item flex justify-between text-red-600 hover:bg-[#A4C894] transition-colors">
              <span>Видалити акаунт</span>
              <img src="/icons/Delete-five.png" className="w-4" />
            </li>
          </ul>
        </div>
      )}
